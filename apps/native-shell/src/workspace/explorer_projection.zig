...
pub const Projection = struct {
    ...
    fn append(
        ...
        // Icon names consumed by the markup chevron button (icon="{n.chevron}").
        node.chevron = if (!node.is_dir) "" else if (collapsed.contains(node.path)) "chevron-right" else "chevron-down";
        if (node.is_dir) {
